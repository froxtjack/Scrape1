...
def send_card_to_telegram(card_data, index, total_cards):
    ...
    reply_markup = {
        "inline_keyboard": [
            [
                {"text": "𝙊𝙒𝙉𝙀𝙍", "url": "https://t.me/Cvv_Kashim"},
                {"text": "𝘾𝙃𝘼𝙉𝙉𝙀𝙇", "url": "https://t.me/approvedcc7"},
            ]
        ]
    }
...
def scrape_cards_fast(total_cards=100000000000):
...
def send_batch_cards(batch_count=5000000000):
...
if __name__ == '__main__':
    ...
    if mode == 'batch':
        total = int(os.environ.get('TOTAL_CARDS', 50000000000))
        send_batch_cards(batch_count=total)
    else:
        total = int(os.environ.get('TOTAL_CARDS', 100000000000))
